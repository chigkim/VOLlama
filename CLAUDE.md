...
`rag/documents.py` is the one place that knows what can be read: `DOCUMENT_EXTENSIONS` and `IMAGE_EXTENSIONS` (the file dialogs build their filters from them), `load()`, `read_files()`, and `fetch_page()`, which tries three readers in order and logs each failure rather than swallowing it. How a wx filter is spelled is `ui/window.py`'s own business and used to be written down here, which put a presentation format in the layer furthest from presentation.
