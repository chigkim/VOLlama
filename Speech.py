...
class Speech:
	def __init__(self):
		...
		self.voice = None
		self.rate = 0.6
		if self.settings.voice != "unknown":
			self.set_voice(self.settings.voice)
			self.set_rate(self.settings.rate)
	...
	def get_voices(self):
		if self.os == 'Darwin':
			voices = [voice.identifier() for voice in AVFoundation.AVSpeechSynthesisVoice.speechVoices()]
			voices.sort()
			return voices
		elif self.os == 'Windows':
