...
class SpeechDialog(wx.Dialog):
	def __init__(self, parent, title, voices, current_voice, rate):
		...
		self.voices_combobox = wx.ComboBox(panel, choices=voices, style=wx.CB_READONLY)
		if current_voice in voices:
			self.voices_combobox.SetValue(current_voice)
		...
		vbox.Add(self.voices_combobox, flag=wx.EXPAND|wx.LEFT|wx.RIGHT, border=10)
	...
	def get_selections(self):
		return self.voices_combobox.GetValue(), self.rate_text_ctrl.GetValue()
